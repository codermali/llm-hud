...
5h   ██░░░░░░░░   24% used  ↻ 14:30    7d   ████░░░░░░   41% used  ↻ Fri 09:00    ctx  ██░░░░░░░░   18% used
```
...
标记本身不发起任何网络请求，也不读取账号凭据；它只比较 Claude Code 报来的数值是否变化。终端太窄时会先舍弃这个标记，再考虑换行。`ctx` 描述当前会话，始终是最新的，因此不参与新鲜度标记。
...
| --- | --- | --- | --- |
| Claude Code | 安装本地状态栏命令 | 模型与推理强度、目录、上下文占用、5 小时与 7 天额度、观测新鲜度 | 基础状态栏 ≥ 1.0.71；完整体验建议 ≥ 2.1.153 |
