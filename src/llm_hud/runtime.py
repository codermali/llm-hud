...
_RUNTIME_CONTENT = ("src", "bin", "README.md", "LICENSE", "pyproject.toml")
_UNSET = object()
...
def validate_runtime(root: Path, release_id: str) -> RuntimeMetadata:
    ...
    if _embedded_runtime_version(path) != runtime_metadata.version:
...
def _embedded_runtime_version(root: Path) -> str:
...
def _finalize_runtime_unlocked(
    ...
    *,
    ...
    embedded_version = _embedded_runtime_version(staging)
